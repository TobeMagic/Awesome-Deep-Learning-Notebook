...
深度学习中常用的归一化技术用于提高模型的训练稳定性和性能。它们在**不同层级**上对输入数据进行归一化，选择**对输入的缩放和偏移不变的归一化方案很重要**。这种选择导致了能够很好地**跨数据集推广**的表示形式。

<img src="https://markdown-1311598839.cos.ap-nanjing.myqcloud.com/img/image-20240227140618869.png" alt="image-20240227140618869" style="zoom: 67%;" />
...
| 名称                                 | 介绍                                                         | 优点                                                         | 缺点                                                         |
| ------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 批归一化（Batch Normalization）      | 批归一化是一种用于在深度学习模型中归一化输入或中间层输出的方法。它通过在训练过程中**对每个批次的数据进行归一化来提高模型的稳定性和训练速度。**批归一化计算每个输入维度的均值和方差，并使用这些统计数据对输入进行中心化和缩放。在训练过程中，批归一化还使用移动平均值和方差来估计整个训练数据集的统计数据，以便在推断阶段使用。 | - 加速模型的训练收敛过程<br>- 提高模型的鲁棒性和稳定性<br>- 减少对初始权重的依赖性<br>- 具有正则化效果，减少过拟合的风险 | - 会受到batchsize大小的影响。如果batchsize太小，算出的均值和方差就会不准确，如果太大，显存又可能不够用。<br>- 对循环神经网络（RNN）的处理相对复杂 |
| 层归一化（Layer Normalization）      | 层归一化是一种对神经网络的中间层输出数据进行归一化的方法。与批归一化不同，层归一化是在**单个样本上进行归一化**，而不是在批次上进行。层归一化计算每个样本在每个输入维度上的均值和方差，并使用这些统计数据对样本进行中心化和缩放。层归一化使得网络对输入数据的大小和均值变化更加鲁棒。 | - 不受batchsize大小的约束，**适用于小批次和单样本的情况**<br>- 提供更好的泛化性能<br>- 对于循环神经网络（RNN）和自注意力机制等递归或并行结构的模型更适用 | - 计算代价较高，在训练和推断过程中都需要计算均值和方差<br>- 不适用于逐样本并行的情况 |
| 组归一化（Group Normalization）      | 组归一化是一种对神经网络的中间层输出数据进行归一化的方法，类似于层归一化。不同之处在于，组归一化**将特征通道分成若干组**，并在每个组内计算均值和方差。组归一化通过对每个样本和每个特征通道组进行归一化来提高模型的鲁棒性。组归一化在计算均值和方差时不依赖于批次大小。 | - 不受batchsize大小的约束，适用于小批次和单样本的情况<br>- 对卷积神经网络（CNN）和循环神经网络（RNN）等模型更适用<br>- 适用于逐样本并行的情况 | - 相对于批归一化，计算代价稍高一些                           |
| 实例归一化（Instance Normalization） | 实例归一化是一种用于神经网络的归一化技术，它对每个样本进行归一化，而不是对批次或层进行归一化。实例归一化计算每个样本在每个通道上的均值和方差，并使用这些统计数据对样本进行中心化和缩放。实例归一化在图像生成和风格迁移等任务中表现出良好的效果。 | - 不依赖于批次大小和层的数量，适用于小批次和单样本的情况<br>- **对于生成模型和风格迁移等任务表现良好**<br>- 提供更好的泛化性能 | - 计算代价较高，在训练和推断过程中都需要计算均值和方差<br>- 对于图像**分类等任务效果**可能不如批归一化和层归一化 |
